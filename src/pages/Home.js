...
import UbicacionFiltro from "../components/UbicacionFiltro";
...
function Home({ searchTerm, searchImg }) {
  ...
  const cambioComboProvincia = (value) => {
    var tam = selectedSize;
    var est = selectedState;
    var prov = value;

    setSelectedProvincia(prov); // Actualiza el estado de selectedState
    const filtro = animals.filter((animal) => {
      var condicion = true;
      condicion =
        tam !== "0" && est !== "0"
          ? animal.estadoAdopcion.toString() === est &&
            animal.tamano.toString() === tam
          : tam !== "0" && est === "0"
          ? animal.tamano.toString() === tam
          : tam === "0" && est !== "0"
          ? animal.estadoAdopcion.toString() === est
    ...
    const filtroP = filtro.filter((animal) => {
      var condicion = true;
      condicion = prov !== "" ? animal.provincia?.toString() === prov : true;
      return condicion;
    });
    setFilteredAnimals(filtroP);
  };
  const cambioComboCanton = (value) => {
    var tam = selectedSize;
    var est = selectedState;
    var prov = selectedProvincia;
    var can = value;

    setSelectedCanton(can); // Actualiza el estado de selectedState
    const filtro = animals.filter((animal) => {
      var condicion = true;
      condicion =
        tam !== "0" && est !== "0"
    ...
    const filtroC = filtro.filter((animal) => {
      var condicion = true;
      condicion =
        prov !== "" && can !== ""
          ? animal.provincia?.toString() === prov &&
            animal.canton?.toString() === can
          : prov !== "" && can === ""
          ? animal.provincia?.toString() === prov
          : true;
      return condicion;
    });
    setFilteredAnimals(filtroC);
  };
  const cambioComboDistrito = (value) => {
    var tam = selectedSize;
    var est = selectedState;
    var prov = selectedProvincia;
    var can = selectedCanton;
    var dis = value;

    setSelectedDistrito(dis); // Actualiza el estado de selectedState
    const filtro = animals.filter((animal) => {
      var condicion = true;
      condicion =
        tam !== "0" && est !== "0"
    ...
    const filtroD = filtro.filter((animal) => {
      var condicion = true;
      condicion =
        prov !== "" && can !== "" && dis !==""
          ? animal.provincia?.toString() === prov &&
            animal.canton?.toString() === can &&
            animal.distrito?.toString() === dis
          : prov !== "" && can !== "" && dis === ""
          ? animal.provincia?.toString() === prov && animal.canton?.toString() === can
          : prov !== "" && can === "" && dis === ""
          ? animal.provincia?.toString() === prov
          : true;
      return condicion;
    });
    setFilteredAnimals(filtroD);
  };
              ...
              <Form>
                ...
                <div className="d-flex flex-wrap gap-3">
                  <UbicacionFiltro
                    ...
                    changeProvincia={(e) => {
                      console.log("Cambio Provincia", e.target.value); // Verifica si se ejecuta el evento
                      cambioComboProvincia(e.target.value);
                    }}
                    changeCanton={(e) => {
                      console.log("Cambio Canton", e.target.value); // Verifica si se ejecuta el evento
                      cambioComboCanton(e.target.value);
                    }}
                    changeDistrito={(e) => {
                      console.log("Cambio Distrito", e.target.value); // Verifica si se ejecuta el evento
                      cambioComboDistrito(e.target.value);
                    }}
                    modCargado={false}
                  />
                </div>
        ...
        {filteredAnimals.map((animal) => (
          <Col md={4} key={animal.id}>
            <Card className="animal-card mb-4">
              ...
              <Card.Body>
                <Card.Title className="animal-title">
                  {animal.nombre}
                </Card.Title>
                ...
                <Card.Text className="animal-breed">
                  {animal.estadoAdopcion === 1
                    ? "En adopción"
                    : animal.estadoAdopcion === 2
                    ? "Adoptado"
                    : animal.estadoAdopcion === 3
                    ? "Perdido"
                    : animal.estadoAdopcion === 4
                    ? "Encontrado"
                    : ""}
                </Card.Text>
                <Card.Text className="animal-breed">
